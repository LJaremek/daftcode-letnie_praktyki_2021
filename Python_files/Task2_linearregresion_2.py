...
sqlite_data = cursor.execute(f'SELECT "transactions"."id", \
    "cards"."created_at", \
    "cards"."exp_year", "cards"."exp_month", \
    ...
    "transactions"."acq_tid", "transactions"."cb" \
...
ca_bin = [row[3] for row in sqlite_data]
ca_last = [row[4] for row in sqlite_data]
ca_country = [row[5] for row in sqlite_data]

cu_created = [row[6] for row in sqlite_data]
cu_name = [row[7] for row in sqlite_data]
cu_email = [row[8].split("@")[1] for row in sqlite_data]
cu_addr = [row[9] for row in sqlite_data]
cu_country = [row[10] for row in sqlite_data]

tr_created = [row[11] for row in sqlite_data]
tr_amount = [row[12] for row in sqlite_data]
tr_currency = [row[13] for row in sqlite_data]
tr_acq = [row[14] for row in sqlite_data]
tr_cb = [row[15] for row in sqlite_data]

reported = [row[16] for row in sqlite_data]
...
ax.set_title("Regresja liniowa po wykluczeniu osób wykonujących przelew z innego kraju oraz nie znaczących parametrów")
plt.show()
